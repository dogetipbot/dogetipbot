...
class HaikuberuCoin(object):
    ...
    def sendtoaddr(self, _userfrom = None, _addrto = None, _amount = None):
        ...
        amount = float(self.verify_amount(_amount=_amount))
    ...
    def get_account_addr(self, _user = None):
        """
        Verify that _user has a coin address
        returns (string) address
        """
        addr = None
        addr = self.conn.getaccountaddress(_user)
        time.sleep(0.5)
    ...
    def verify_amount(self, _amount = None):
        ...
        if not _amount or not type(_amount) in [Decimal, int, float] or not _amount > 0:
