...
class HkExchange(object):
    ...
    def __init__(self, _conf = None):
        """
        Initialize HkExchange with given parameters.
        ...
        if not _conf or not hasattr(_conf, 'urlpaths') or not hasattr(_conf, 'jsonpaths') or not hasattr(_conf, 'coinlist') or not hasattr(_conf, 'fiatlist'):
            raise Exception("HkExchange::__init__(): _conf is empty or invalid")
        ...
        lg.debug("HkExchange::__init__(): initialized exchange %s" % self.conf.domain)
    ...
    def supports(self, _name = None):
        ...
        if not _name or not type(_name) in [str, unicode]:
            raise Exception("HkExchange::supports(): _name is empty or wrong type")

        name = str(_name).lower()

        if name in self.conf.coinlist or name in self.conf.fiatlist:
            #lg.debug("HkExchange::supports(%s): YES" % name)
            return True
        else:
            #lg.debug("HkExchange::supports(%s): NO" % name)
            return False
    ...
    def get_ticker_value(self, _name1 = None, _name2 = None):
        ...
        if not self.supports_pair(_name1=_name1, _name2=_name2):
            raise Exception("HkExchange::get_ticker_value(%s, %s, %s): pair not supported" % (self.conf.domain, _name1, _name2))
        ...
        for myurlpath in self.conf.urlpaths:
            for myjsonpath in self.conf.jsonpaths:
                ...
                try:
                    lg.debug("HkExchange::get_ticker_value(%s, %s, %s): calling %s to get %s...", self.conf.domain, _name1, _name2, myurlpath, myjsonpath)
                    if self.conf.https:
                    ...
                    lg.debug("HkExchange::get_ticker_value(%s, %s, %s): result: %.8f", self.conf.domain, _name1, _name2, float(result))
                    results.append( float(result) )

                except urllib2.URLError as e:
                    lg.error("HkExchange::get_ticker_value(%s, %s, %s): %s", self.conf.domain, _name1, _name2, e)
                    return None
